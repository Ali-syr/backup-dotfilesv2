...
Singleton {
    ...
    function applyIconTheme(theme) {
        ...
        // Reload Quickshell so IconImage re-resolves paths from the new theme
    ...
    // Writes every cursor sink: gsettings + ~/.local/share/icons/default + niri (qssettings/shell.kdl if ready)
    ...
    Process {
        ...
        command: ["bash", "-c", `
            echo :::ICON
            find "$HOME/.local/share/icons" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | xargs -r -n1 basename | sort -u | grep -Evx 'hicolor|default'
            echo :::GTK
